...
jQuery(function($){
    var h2       = $('.editor-area').find('h2'),
        h2Count  = h2.length,
        h2Text   = new Array(h2Count),
        forCount = h2Count - 1;

    $('.editor-area .entry-body').prepend('<h2>もくじ</h2><ul id="midashi"/>');
    for (var i = 0; i < h2Count; i++) {
        $('#midashi').append('<li><a/></li>');
        $('#midashi').children('li').children('a').eq(i).text(h2.eq(i).text());
    }

    for (var i = 0; i < h2Count; i++) {
        var idName = 'h2-' + i;
        h2.eq(i).attr('id',idName);
        $('#midashi').children('li').children('a').eq(i).attr('href', '#' + idName);
    }
});
